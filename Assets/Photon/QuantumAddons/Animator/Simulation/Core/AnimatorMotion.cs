namespace Quantum.Addons.Animator
{
  ...
  public abstract unsafe class AnimatorMotion : IAnimatorMotion
  {
    ...
    public virtual void ProcessEvents(Frame f, AnimatorComponent* animator, LayerData* layerData, int stateId, AnimatorStateType stateType)
    {
    }
  ...
  public unsafe class AnimatorClip : AnimatorMotion
  {
    ...
    public override void ProcessEvents(Frame f, AnimatorComponent* animator, LayerData* layerData, int stateId, AnimatorStateType stateType)
    {
      if (Data.Events == null)
      {
        return;
      ...
      foreach (var animatorEvent in Data.Events)
      {
        ...
        {
          animatorEvent.Evaluate(f, animator, layerData, stateType);
        }
      }
    }
  ...
  public unsafe class AnimatorBlendTree : AnimatorMotion
  {
    ...
    public override void ProcessEvents(Frame f, AnimatorComponent* animator, LayerData* layerData, int stateId, AnimatorStateType stateType)
    {
      ...
      for (int i = 0; i < Motions.Length; i++)
      {
        ...
        if (clipWeight > 0)
        {
          Motions[i].ProcessEvents(f, animator, layerData, stateId, stateType);
        }
      }
    }
      ...
      int stateId)
    {
      ...
      else
      {
        ...
        int indexB = indexA + 1;
        lerp = lerp - (indexA - blendParameter);
      ...
      out FP speed)
    {
      ...
      {
        var firstPoint = Positions[0].X;
        var lastPoint = Positions[^1].X;
        var blendVariable = FPMath.Clamp(
          *AnimatorComponent.Variable(f, animatorComponent, BlendParameterIndex)->FPValue,
          firstPoint, lastPoint);
        var blendAmplitude = lastPoint - firstPoint;
