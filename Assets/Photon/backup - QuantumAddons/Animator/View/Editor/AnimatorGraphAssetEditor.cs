namespace Quantum.Addons.Animator
{
  ...
  public class AnimatorGraphAssetEditor : Editor
  {
    ...
    private void Export(object obj)
    {
      try
      {
        Type exportType = (Type)obj;

        var instance = Activator.CreateInstance(exportType);
        exportType.GetMethod("BakeGraph").Invoke(instance, new object[] { _asset });
      }
      catch (Exception e)
      {
    ...
    public override void OnInspectorGUI()
    {
      ...
      {
        // MD:  We look for any class that is inheriting from AnimatorGraphBaker.
        var exportSubClasses =
          new List<Type>(typeof(AnimatorGraphBaker).GetSubClasses(AppDomain.CurrentDomain.GetAssemblies()));

        // If we only find one (ExportAnimations), we just use the default export animations call
        if (exportSubClasses.Count <= 1)
        {
          try
          {
            AnimatorGraphBaker exporter = new AnimatorGraphBaker();
            exporter.BakeGraph(_asset);
          }
          catch (Exception e)
          {
            Debug.LogException(e);
        ...
        {
          // Otherwise, we create a generic menu and let the user pic which exporter they want to use.
          GenericMenu exportMenu = new GenericMenu();
          for (int i = 0; i < exportSubClasses.Count; i++)
          {
            exportMenu.AddItem(new GUIContent($"Bake Using {exportSubClasses[i].Name}"), false, Export,
              exportSubClasses[i]);
          }

          exportMenu.ShowAsContext();
        }
      ...
      if (_showDetailedData)
      {
        if (graph != null && graph.IsValid && graph.Layers != null)
        {
          ...
          if (graph.RootMotion && !graph.LegacyRootMotionExtractor && graph.ReferenceModel == null)
          {
            ...
            GUILayout.Label($"Reference Model is note set, using legacy Root Motion extractor.", warningStyle,
              GUILayout.Height(50));
                ...
                if (state != null)
                {
                  ...
                  if (_showTransitions = EditorGUILayout.Foldout(_showTransitions, "Transitions"))
                  {
                    ...
                    for (int transitionIndex = 0; transitionIndex < transitionCount; transitionIndex++)
                    {
                      ...
                      EditorGUILayout.LabelField(string.Format("Has ExitState Time: {0}", transition.HasExitTime));
                      EditorGUILayout.LabelField(string.Format("ExitState Time: {0} sec", transition.ExitTime.AsFloat));
    ...
    private void AnimationClipGui(AnimatorClip clip)
    {
      ...
      EditorGUILayout.LabelField(string.Format("Disable Root Motion: {0}", clip.Data.DisableRootMotion));
      ...
      float[] rotationsY = new float[frameCount];
      ...
      for (int f = 0; f < frameCount; f++)
      {
        ...
        rotationsQ[f] = rQ;
        rotations[f] = rV3;
        rotationsY[f] = frame.RotationY.AsFloat;
        times[f] = frameTime;
      }

      EditorGUILayout.BeginVertical("box");

      EditorGUILayout.LabelField($"Delta Movement: {(positions[frameCount - 1] - positions[0]).ToString("F3")}");
      ...
      EditorGUILayout.LabelField($"Delta Rotation: {deltaQV.ToString("F3")}");

      EditorGUILayout.LabelField($"Delta RotationY: {(rotationsY[frameCount - 1] - rotationsY[0]):F3}");
      ...
      if (_showMovementData = EditorGUILayout.Foldout(_showMovementData, "Movement Data"))
      {
        ...
        EditorGUILayout.BeginVertical();
        EditorGUILayout.LabelField("RotationsY", GUILayout.Width(160));
        for (int f = 0; f < frameCount; f++)
          EditorGUILayout.LabelField(rotationsY[f].ToString("F2"), GUILayout.Width(75));
        EditorGUILayout.EndVertical();
